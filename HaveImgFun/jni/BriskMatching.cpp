...
BriskMatching::BriskMatching(void):ratio(0.75f),distance(2.0),confidence(0.98),refineF(true) //ratio=0.8, confidence=0.98
{
}


BriskMatching::~BriskMatching(void)
{
}


void BriskMatching::setRatio(float rt)
{
	ratio = rt;
}


void BriskMatching::setDistance(double dst)
{
	distance = dst;
}


void BriskMatching::setConfidence(double confd)
{
	confidence = confd;
}
...
int BriskMatching::ratioTest(vector< vector< DMatch > > &matches)
{
	int removed = 0;
...
void BriskMatching::symmetryTest(const vector< vector< DMatch > >& matches1,const vector< vector< DMatch > >& matches2, vector< DMatch >& symMatches)
{
...
void BriskMatching::ransacTest(const vector< DMatch >& matches, const vector< KeyPoint >& keypoints1, const vector< KeyPoint >& keypoints2, vector< DMatch >& outMatches)
{
...
void BriskMatching::filterWithSlopeDistance(	Mat& image1,
											Mat& image2,
...
vector< DMatch > BriskMatching::DoMatching(Mat& image1, Mat& image2, double &time)
{
